"""
Simple, reliable football-data.org API collector
"""

import asyncio
import logging
import os
import aiohttp
from datetime import datetime, timedelta
from typing import List, Dict, Optional

logger = logging.getLogger(__name__)

class SimpleFootballAPI:
    """Simple and reliable football data collector"""
    
    def __init__(self):
        self.api_key = os.getenv('FOOTBALL_API_KEY')
        self.base_url = "https://api.football-data.org/v4"
        
        # Focus on main leagues that always have matches
        self.leagues = {
            'Premier League': 'PL',
            'La Liga': 'PD', 
            'Serie A': 'SA',
            'Bundesliga': 'BL1',
            'Ligue 1': 'FL1'
        }
    
    async def initialize(self):
        """Initialize method for compatibility"""
        logger.info("Simple Football API initialized")
    
    async def close(self):
        """Close method for compatibility"""
        logger.info("Simple Football API closed")
    
    async def get_upcoming_matches(self) -> List[Dict]:
        """Get upcoming matches from football-data.org"""
        return await self._fetch_matches()
    
    async def get_real_upcoming_matches(self) -> List[Dict]:
        """Compatibility method - same as get_upcoming_matches"""
        return await self._fetch_matches()
    
    async def _fetch_matches(self) -> List[Dict]:
        """Get upcoming matches from football-data.org"""
        if not self.api_key:
            logger.error("FOOTBALL_API_KEY not found")
            return []
        
        matches = []
        today = datetime.now()
        end_date = today + timedelta(days=7)
        
        headers = {
            'X-Auth-Token': self.api_key,
            'Content-Type': 'application/json'
        }
        
        for league_name, league_code in self.leagues.items():
            try:
                logger.info(f"Fetching {league_name} fixtures...")
                
                url = f"{self.base_url}/competitions/{league_code}/matches"
                params = {
                    'dateFrom': today.strftime('%Y-%m-%d'),
                    'dateTo': end_date.strftime('%Y-%m-%d'),
                    'status': 'SCHEDULED'
                }
                
                # Create fresh session for each request - no reuse
                timeout = aiohttp.ClientTimeout(total=10)
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.get(url, headers=headers, params=params) as response:
                        if response.status == 200:
                            data = await response.json()
                            league_matches = data.get('matches', [])
                            
                            for match in league_matches:
                                parsed_match = self._parse_match(match, league_name)
                                if parsed_match:
                                    matches.append(parsed_match)
                            
                            logger.info(f"✓ Got {len(league_matches)} matches from {league_name}")
                        
                        elif response.status == 429:
                            logger.warning(f"Rate limit for {league_name}, waiting...")
                            await asyncio.sleep(60)
                        
                        else:
                            logger.warning(f"API returned {response.status} for {league_name}")
                
                # Small delay between requests
                await asyncio.sleep(0.5)
                
            except Exception as e:
                logger.error(f"Error fetching {league_name}: {e}")
                continue
        
        # Sort by date
        matches.sort(key=lambda x: x.get('date', ''))
        logger.info(f"Total matches retrieved: {len(matches)}")
        return matches
    
    def _parse_match(self, match: Dict, league_name: str) -> Optional[Dict]:
        """Parse match data"""
        try:
            home_team = match.get('homeTeam', {}).get('name', 'TBD')
            away_team = match.get('awayTeam', {}).get('name', 'TBD')
            
            if not home_team or not away_team or home_team == 'TBD' or away_team == 'TBD':
                return None
            
            # Parse date
            utc_date = match.get('utcDate', '')
            match_date = datetime.now().strftime('%Y-%m-%d')
            match_time = 'TBD'
            
            if utc_date:
                try:
                    parsed_datetime = datetime.fromisoformat(utc_date.replace('Z', '+00:00'))
                    match_date = parsed_datetime.strftime('%Y-%m-%d')
                    match_time = parsed_datetime.strftime('%H:%M')
                except:
                    pass
            
            return {
                'home_team': home_team,
                'away_team': away_team,
                'league': league_name,
                'match_time': match_time,
                'date': match_date,
                'source': 'Football-Data.org'
            }
            
        except Exception as e:
            logger.debug(f"Error parsing match: {e}")
            return None